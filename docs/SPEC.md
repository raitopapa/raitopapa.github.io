...
- **樹木管理ナビブランド** — E-E-A-T の "Expertise" が確立済み
...
- 理由: 樹木管理ナビブランドとAnthropic技術スタックの整合、現状PWAクライアントと自然な接続
...
- **詳細**: `/glossary/`（または Jekyll collection）。ABC順・五十音順タブ。各用語に短い定義・関連ツールリンク。診断結果から該当用語へリンク。
...
- **詳細**: 無料は履歴30日・PDFに「樹木管理ナビ」透かし。Pro は無制限・透かし無し・複数物件。`assets/js/feature-flags.js` で集中管理。
...
| D4 | ブランド名 | ✅ **確定**: 「樹木管理ナビ」へ変更。樹木医はサイト名ではなく監修・権威付けとして使う |
...
**未決定（Gate 通過時 or 段階達成時に確定）**: D2、D3、D6
...
| 2026-05-03 | D1-D10 仮決定 → 最終確定 | raitopapa 承認 | **D1/D5/D7/D8/D9/D10 確定、D2/D3/D6 はゲート時保留継続** | 本SPEC §8 |
| 2026-05-03 | D4 ブランド名を「樹木管理ナビ」に確定 | raitopapa 承認 | 採用。樹木医は監修・権威付けとして使用 | 本SPEC §8 |
| 2026-05-03 | 本体 docs/ 統合は Codex 担当 | raitopapa 指示 | 採用 | 本SPEC §12 |
| 2026-05-03 | コスト上限 ¥500/月 を強制制約 | raitopapa 指示 | 採用、§7.1 新設 | 本SPEC §7.1 |

未決事項: D2（価格帯）・D3（AI診断）・D6（監修印）— いずれも Phase 3 ゲート通過 or D5 第2段階達成後に確定
