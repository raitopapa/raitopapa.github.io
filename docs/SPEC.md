...
- **樹木ナビブランド** — E-E-A-T の "Expertise" が確立済み
...
- 理由: 樹木ナビブランドとAnthropic技術スタックの整合、現状PWAクライアントと自然な接続
...
- **詳細**: 無料は履歴30日・PDFに「樹木ナビ」透かし。Pro は無制限・透かし無し・複数物件。`assets/js/feature-flags.js` で集中管理。
...
| D4 | ブランド名 | ✅ **確定**: 「樹木ナビ」へ変更。樹木医はサイト名ではなく監修・権威付けとして使う |
...
| 2026-05-03 | D4 ブランド名を「樹木ナビ」に確定 | raitopapa 承認 | 採用。樹木医は監修・権威付けとして使用 | 本SPEC §8 |
